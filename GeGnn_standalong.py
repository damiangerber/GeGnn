...
class PretrainedModel(nn.Module):
    def __init__(self, ckpt_path):
        ...
        self.model = hgraph.models.graph_unet.GraphUNet(6, 256).to(self.device)
        self.embds = None
    ...
    def precompute(self, mesh):
        with torch.no_grad():
            ...
            tree.build_single_hgraph(
                hgraph.hgraph.Data(
                    x=torch.cat([vertices, normals], dim=1), edge_index=edges
                )
            )
...
def main():
    ...
    parser.add_argument(
        "--test_file", type=str, default=None, help="path to the obj file"
    )
    parser.add_argument(
        "--ckpt_path", type=str, default=None, help="path to the checkpoint"
    )
    parser.add_argument(
        "--start_pts", type=str, default=None, help="an int is expected."
    )
    parser.add_argument(
